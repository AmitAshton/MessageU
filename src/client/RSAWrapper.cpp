...
void RSAPrivateWrapper::loadFromString(const std::string& key)
{
	CryptoPP::StringSource ss(key, true);
	_privateKey.Load(ss);
}

void RSAPrivateWrapper::generate()
{
	_privateKey.Initialize(_rng, BITS);
}

bool RSAPrivateWrapper::hasKey() const
{
	// Validate requires a non-const RNG reference; create one locally and pass it
	CryptoPP::AutoSeededRandomPool rng;
	return _privateKey.Validate(rng, 3);
}
